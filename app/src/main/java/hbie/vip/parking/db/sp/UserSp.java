package hbie.vip.parking.db.sp;
...
public class UserSp extends BaseSp<UserInfo> {
    ...
    public void read(UserInfo user) {
        ...
        if(getSP().contains("userCardid")){
            user.setUserCardid(getSP().getString("userCardid",""));
        }
...
//        private String userCurrentCarEnginenumber;
    ...
    public void write(UserInfo user) {
        ...
        if(!user.getUserCardid().equals("")){
            editor.putString("userCardid",user.getUserAddr());
        }
    ...
    public void clearCar(){//删除容器中车辆的信息
        SharedPreferences.Editor editor = getSP().edit();
        editor.remove("userCurrentCarId");//这里面填写车辆保存的KEY
        editor.remove("userCurrentCarBrand");
        editor.remove("userCurrentCarType");
        editor.remove("Currentcar");
        editor.remove("userCurrentCarEnginenumber");
        editor.commit();
